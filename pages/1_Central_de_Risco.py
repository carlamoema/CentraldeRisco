...
with tab1: # Goiás
     ...
     with st.container():
          ...
          df_aux1 = df_aux.sort_values('mean', ascending=False).head(5).reset_index()
          fig = go.Figure()
          fig.add_trace(go.Bar(name='Control', x=df_aux1['ocupacao'], y=df_aux1['Media'], error_y=dict(type='data', array=df_aux1['Desvio_padrao'])))
          st.plotly_chart(fig, use_container_width=True)
          #return fig
