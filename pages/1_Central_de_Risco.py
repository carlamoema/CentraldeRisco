...
with tab1: # Goiás
     ...
     with st.container():
          ...
          df_aux=df4.loc[:,cols ].groupby('ocupacao').agg({'carteira_ativa': ['sum', 'mean', 'std', lambda x: stats.mode(x.round())[0][0]], 'numero_de_operacoes': ['sum'], 'cliente': ['count'] }).reset_index()
